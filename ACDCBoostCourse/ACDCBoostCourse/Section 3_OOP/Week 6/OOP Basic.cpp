...
struct Transaction {
    ...
    //Attributes
    //구조체의 정보(properties)
    transactionName_t transactionName;  //거래 이름
    won_t depositAndWithdrawal;         //입출금 금액

}; // struct는 public이 기본값

// 3. DayOfTheWeek Enum 정의
enum DayOfTheWeek { // 열거형 정의
...
public:
    ...
    ~BankAccount() {
        ...
        // 이 예시에는 멤버변수에 동적 할당된 것이 없으므로 따로 해제해 줄 것이 없음.
    }

    // Methods 메서드: 클래스 안에서의 함수
    name_t getName() {
        return clientName;
    }
    ...
    int getPassword() {
     ...
     // private인 정보를 참조하기 위해 getter 사용
